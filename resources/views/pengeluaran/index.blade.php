...
@section('isi')
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-head-fixed text-nowrap" style="text-align:center">
                ...
                <tbody>
                    @forelse ($model as $rowPengeluaran)
                        <tr>
                            <td>{{ date('d-m-Y', strtotime($rowPengeluaran->tanggal)) }}</td>
                            <td>{{ Kategori::where('kategori_id', $rowPengeluaran->kategori_id)->first()->nama_kategori }}
                            </td>
                            <td>{{ $rowPengeluaran->keterangan }}</td>
                            <td>{{ 'Rp ' . number_format($rowPengeluaran->nominal, 0, ',', '.') . ',-' }}</td>
                            <td>
                                <button class="btn btn-sm btn-danger"
                                    onclick="hapusPengeluaran({{ $rowPengeluaran->pengeluaran_id }})">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                    ...
                    <tr>
                        ...
                        <td id="total_pengeluaran">{{ 'Rp ' . number_format($total, 0, ',', '.') . ',-' }}</td>
                        <td></td>
                    </tr>
                </tbody>
            </table>
    ...
    <div class="modal fade" id="modal-tambah-data">
        <form action="{{ url('/simpanPengeluaran') }}" method="POST">
            @csrf
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Tambah Transaksi Pengeluaran</h3>
                            </div>
                            <div class="card-body">
...
@section('js')
    ...
    <script>
        function hapusPengeluaran(pengeluaran_id) {
            if (confirm('Apakah Yakin data ini akan dihapus ?')) {
                location.href = "{{ url('/deletePengeluaran') }}/" + pengeluaran_id;
            }
        }
